...
app.engine('handlebars', exphbs({
  ...
  helpers: {
   ...
   // Convert JSON data and return as JSONstring.
    ...
    OpenChromeCheck(){
      let launchChrome = config.general.launchchromeatstartup || true;
      if (launchChrome){
...
if (process.pkg) {
  // Si se ejecuta como un binario empaquetado con pkg
  app.set('views', path.join(process.cwd(), 'views'));
  app.use(express.static(path.join(process.cwd(), 'static')));
} else {
  // Si se ejecuta en entorno de desarrollo normal
