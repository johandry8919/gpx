    ...
    gapi.client.sheets.spreadsheets.get({
        ...
        for (var i = 0; i < sheets.length; i++) {
            ...
            gapi.client.sheets.spreadsheets.values.get({
                ...
                var regExp = /\(([^)]+)\)/;
                $('#bx-player-1 .name').text(valueData['EQUIPO'][25]);
                $('#bx-player-2 .name').text(valueData['EQUIPO'][26]);
                var clasesA = valueData['EQUIPO'][25];
                var clasA = clasesA.toLowerCase();
                var clasesB = valueData['EQUIPO'][26];
                ...
                $('#bat-m-A').html(smAC);
                $('#bat-m-B').html(smBC);
                if(valueData['TITLE'][25] !== 'undefined'){
                    $('.sponsor-wrap img').attr('src',valueData['TITLE'][25]);
                    $('.sponsor-wrap').show();
                }
            }, function (error) {
                console.log(error);
