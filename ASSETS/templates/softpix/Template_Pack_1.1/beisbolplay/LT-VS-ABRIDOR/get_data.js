    ...
    gapi.client.sheets.spreadsheets.get({
        ...
        for (var i = 0; i < sheets.length; i++) {
            ...
            gapi.client.sheets.spreadsheets.values.get({
                ...
                $('#bx-player-1 .name').text(valueData['JUGADOR'][13]);
                $('#bx-player-2 .name').text(valueData['JUGADOR'][14]);
                var clasesA = valueData['EQUIPO'][13];
                var clasA = clasesA.toLowerCase();
                var clasesB = valueData['EQUIPO'][14];
                ...
                getPlayerA(valueData['CODE'][13]);
                getPlayerB(valueData['CODE'][14]);
...
xmlhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
        ...
        person.map((val)=>{
          if(val.season === "2023" ){
            $('#bat-GP-A').text(val.stat.wins+'/'+val.stat.losses);
...
xmlhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
        ...
        person.map((val)=>{
          if(val.season === "2023" ){
            $('#bat-GP-B').text(val.stat.wins+'/'+val.stat.losses);
